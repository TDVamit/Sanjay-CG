...
const GuidanceAgents: React.FC = () => {
  ...
  const loadAgents = async (page: number = 1, search?: string, categoryId?: string) => {
    try {
      setLoading(true);
      const response = await guidanceAgentAPI.getAll(page, 12, search, categoryId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use form data as is without special category modification
  ...
  const handleEdit = (agent: GuidanceAgentResponse) => {
    setFormData({
      ...
      link: agent.link,
      category_ids: agent.categories.map(cat => getCategoryId(cat) || ''),
      profile_pic: undefined
    });
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
            {agents.map(agent => (
              <div 
                key={agent._id} 
                className="bg-neutral-900/60 backdrop-blur-md border border-neutral-700/30 rounded-lg p-4 sm:p-6 hover:border-neutral-600/30 transition-all duration-300 hover:scale-105 hover:shadow-xl group"
                style={{ 
              ...
              >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-base sm:text-lg font-semibold text-white mb-1 group-hover:text-neutral-300 transition-colors line-clamp-2">{agent.name}</h3>
                  ...
                  <div className="flex flex-wrap gap-1 mb-4">
                    ...
                    {agent.categories.length > 2 && (
                      <span className="px-2 py-1 bg-neutral-600/20 border border-neutral-400/30 text-neutral-400 rounded text-xs">
                        +{agent.categories.length - 2} more
                      </span>
                    )}
                  </div>
                )}
                ...
                <div className="flex items-center justify-between">
                  <a
                    ...
                    className="flex items-center space-x-2 px-3 sm:px-4 py-2 bg-neutral-700 text-white rounded-lg hover:bg-neutral-600 transition-colors text-xs sm:text-sm"
                  >
                    <span>Visit</span>
